...
export default function NuevaPropiedadPage() {
  ...
  const handleImageChange = (e) => {
    ...
    // Validar cantidad máxima de imágenes (ejemplo: 10)
    if (imagePreviews.length + files.length > 10) {
      alert('Máximo 10 imágenes permitidas');
      return;
    }

    // Validar tamaño de cada archivo (ejemplo: 5MB)
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4 justify-end">
          ...
          <button
            type="submit"
            disabled={loading}
            className="flex items-center gap-2 bg-cerro-verde hover:bg-xela-navy text-white px-6 py-3 rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
